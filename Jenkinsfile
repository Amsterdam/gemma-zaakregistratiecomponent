...
node {
    ...
    stage("Build image") {
        tryStep "build", {
            def image = docker.build("build.datapunt.amsterdam.nl:5000/gemma-zrc:${env.BUILD_NUMBER}")
            image.push()
    ...
    node {
        stage('Push acceptance image') {
            tryStep "image tagging", {
                def image = docker.image("build.datapunt.amsterdam.nl:5000/gemma-zrc:${env.BUILD_NUMBER}")
                image.pull()
